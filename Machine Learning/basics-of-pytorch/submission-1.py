...
class Solution:
    def reshape(self, to_reshape: TensorType[float]) -> TensorType[float]:
        ...
        reshaped = torch.reshape(to_reshape, ((int)(M*N/2), 2))
        return torch.round(reshaped, decimals=4)

    def average(self, to_avg: TensorType[float]) -> TensorType[float]:
        # Compute column-wise mean (average across rows)
        # Use torch.mean(tensor, dim=0)
        tensor_mean = torch.mean(to_avg, dim = 0)
        return torch.round(tensor_mean, decimals=4)
    ...
    def get_loss(self, prediction: TensorType[float], target: TensorType[float]) -> TensorType[float]:
        ...
        mse_tensor = torch.nn.functional.mse_loss(prediction, target)
        return torch.round(mse_tensor, decimals=4)
